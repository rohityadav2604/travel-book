...
export default function HighlandJournal({ leftPhotoUrl, quote, date, texts }: HighlandJournalProps): React.ReactElement {
  ...
  return (
    <HPageBg>
      <div style={{ position: "absolute", top: 0, bottom: 0, left: 0, width: 260 }}>
        ...
        <div style={{ position: "absolute", bottom: 30, left: 24, right: 24, color: "#b8c2a3" }}>
          <div className="m-mono" style={{ fontSize: 9, letterSpacing: ".15em" }}>
            {date ?? texts?.date ?? "OCT · 18 · 14h 06"}
          </div>
          <div className="m-mono" style={{ fontSize: 9, letterSpacing: ".15em", marginTop: 4, opacity: 0.8 }}>
            {texts?.weather ?? "NW 22kt · -2°C"}
          </div>
        </div>
      </div>

      <div style={{ position: "absolute", top: 56, left: 290, right: 56, bottom: 56 }}>
        <div className="m-caps" style={{ fontSize: 9, color: "#4a6346", letterSpacing: ".3em" }}>{texts?.sectionLabel ?? "FIELD JOURNAL"}</div>
        ...
        <p className="m-serif" style={{ fontSize: 13, lineHeight: 1.7, color: "#1a2218", margin: "20px 0 0", textWrap: "pretty", fontStyle: "italic" }}>
          {quote ?? texts?.quote ?? "Crossed the saddle in low cloud. The cairns kept their distance and the wind kept ours."}
        </p>
        <p className="m-script" style={{ fontSize: 17, lineHeight: 1.4, color: "#3a4438", margin: "14px 0 0", textWrap: "pretty" }}>
          {texts?.body ?? "Saw a chamois. Did not photograph it."}
        </p>

        <div style={{ marginTop: 22, paddingTop: 14, borderTop: "1px solid #6b7568" }}>
          <div className="m-caps" style={{ fontSize: 9, color: "#4a6346", letterSpacing: ".25em" }}>{texts?.sightingsLabel ?? "SIGHTINGS"}</div>
          <div className="m-mono" style={{ fontSize: 10, color: "#3a4438", marginTop: 8, lineHeight: 1.7 }}>
            {sightingsLines.map((line, index) => (
              <React.Fragment key={`${line}-${index}`}>
                {line}
                {index < sightingsLines.length - 1 ? <br /> : null}
              </React.Fragment>
            ))}
          </div>
      ...
      <div className="m-pagenum">{texts?.pageNumber ?? "— 41 —"}</div>
    </HPageBg>
  );
}
